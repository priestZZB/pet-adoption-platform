...
const routes = [
  ...
  { path: '/register',       component: () => import('../views/user/RegisterPage.vue') },
  ...
  // ===== 主布局（MobileShell：顶栏 + 内容 + 底栏 TabBar）=====
  {
    path: '/',
    ...
    children: [
      ...
      { path: 'addresses',              component: () => import('../views/mall/AddressManage.vue'),    meta: { requiresAuth: true } },
    ]
  },
...
router.beforeEach(async (to, from) => {
  ...
  if (token && to.path !== '/login' && to.path !== '/register' && to.path !== '/reset-password') {
    ...
    if (userStore.isAdmin) {
      const path = to.path
      const allowed = ['/', '/mall', '/ai', '/notices', '/admin', '/user/profile', '/user/reviews']
      const allowedPrefix = ['/admin/', '/mall/products/', '/pets/', '/notices/', '/user/profile/', '/user/review/']
  ...
  if (to.meta.role) {
    const userStore = useUserStore()
    if (token) { await userStore.fetchUserInfo() }
