<template>
  <div class="page">
    ...
    <MobileEmpty v-else-if="list.length === 0" icon="fas fa-clipboard-check" description="暂无待审核宠物" />
    <div v-else class="list">
      <MobileCard v-for="pet in list" :key="pet.id" class="item" clickable @click="goReview(pet.id)">
        <div class="pet-row">
          <img :src="pet.coverImage" class="pet-img" />
          <div class="pet-info">
            <h4>{{ pet.name }}</h4>
            <p>{{ pet.breed }} · {{ pet.age }}</p>
            <span class="gender">{{ genderMap[pet.gender] || '-' }}</span>
          </div>
...
onMounted(async () => { try { list.value = await request.get('/volunteer/pets/pending') } catch {} finally { loading.value = false } })
function goReview(id) { router.push('/volunteer/review/' + id) }
</script>
...
.gender { font-size: 12px; color: #8ab8a0; }
