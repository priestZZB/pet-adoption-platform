    ...
    <div class="card-wrap">
      <MobileCard>
        <div class="apply-header">
          ...
          <h2>申请成为志愿者</h2>
          <p>志愿者可参与宠物审核、回访记录等工作，为流浪动物贡献一份力量。</p>
        </div>
...
async function loadStatus() { try { status.value = await getVolunteerStatus() } catch {} }

async function handleApply() {
  applying.value = true
  try { await volunteerApply(); ElMessage.success('申请已提交'); loadStatus() } catch {} finally { applying.value = false }
}
