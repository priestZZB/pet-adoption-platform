<template>
  <div class="page">
    <div class="card-wrap">
      <MobileCard>
        <div class="form-group"><label>新手机号</label><el-input v-model="form.phone" placeholder="输入新手机号" maxlength="11" /></div>
        ...
        <button class="save-btn" :disabled="submitting" @click="handleSubmit">{{ submitting ? '提交中...' : '换绑手机' }}</button>
      </MobileCard>
    </div>
  </div>
</template>
...
async function handleSendSms() {
  ...
  try { await sendSmsCode({ phone: form.phone }); ElMessage.success('验证码已发送'); smsCountdown.value = 60; smsTimer = setInterval(() => { smsCountdown.value--; if (smsCountdown.value <= 0) { clearInterval(smsTimer); smsTimer = null } }, 1000) } catch {} finally { smsSending.value = false }
}

async function handleSubmit() {
  submitting.value = true
  try { await updateUserInfo({ phone: form.phone, smsCode: form.smsCode }); ElMessage.success('手机号换绑成功') } catch {} finally { submitting.value = false }
}
</script>
