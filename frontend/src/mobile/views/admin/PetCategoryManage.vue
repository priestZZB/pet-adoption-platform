...
async function loadData() { loading.value = true; try { list.value = await getCategories() } catch {} finally { loading.value = false } }
...
async function handleSave() {
  try {
    if (editId.value) { await updatePetCategory(editId.value, form.name) } else { await addPetCategory(form.name) }
    ElMessage.success('保存成功'); dialogVisible.value = false; loadData()
  } catch {}
}
async function handleDelete(id) {
  try { await ElMessageBox.confirm('确定删除？'); await deletePetCategory(id); ElMessage.success('已删除'); loadData() } catch {}
}
onMounted(loadData)
</script>
...
.add-btn { height: 36px; padding: 0 16px; border: 1px dashed #409EFF; border-radius: 8px; background: none; color: #409EFF; font-size: 14px; cursor: pointer; width: 100%; }
