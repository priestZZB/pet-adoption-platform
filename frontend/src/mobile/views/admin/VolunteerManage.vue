<template>
  <div class="page">
    ...
    <MobileEmpty v-else-if="list.length === 0" description="暂无志愿者申请" />
    <div v-else class="list">
      <MobileCard v-for="v in list" :key="v.id" class="item">
        <div class="row"><span class="name">{{ v.nickname || v.username }}</span><span :class="['status', statusClass(v.status)]">{{ v.status || '待审核' }}</span></div>
        <div class="meta">{{ v.phone }} | {{ v.createdAt?.slice(0, 10) }}</div>
        <div class="acts" v-if="v.status === '待审核'">
          <button class="approve" @click="handleReview(v.id, 'approve')">通过</button>
          <button class="reject" @click="openReject(v)">驳回</button>
        </div>
      </MobileCard>
    </div>
...
import { getVolunteerApplies, reviewVolunteer } from '@/api/admin'
...
onMounted(async () => { try { list.value = await getVolunteerApplies() } catch {} finally { loading.value = false } })

function statusClass(s) { const m = { '待审核': 'warn', '已通过': 'ok', '已驳回': 'err' }; return m[s] || '' }
function openReject(v) { rejectId.value = v.id; rejectReason.value = ''; rejectVisible.value = true }
async function handleReview(id, action) {
  try { await reviewVolunteer(id, { action }); ElMessage.success('操作成功'); list.value = list.value.filter(i => i.id !== id) } catch {}
}
async function confirmReject() {
  try { await reviewVolunteer(rejectId.value, { action: 'reject', reason: rejectReason.value }); ElMessage.success('已驳回'); rejectVisible.value = false; list.value = list.value.filter(i => i.id !== rejectId.value) } catch {}
}
</script>
