    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑分类' : '新增分类'" width="400px">
      <el-form :model="form" label-width="80px">
        <el-form-item label="名称" required>
          <el-input v-model="form.name" placeholder="如：猫、狗" />
        </el-form-item>
...
async function handleSave() {
  ...
  try {
    if (isEdit.value) {
      await updatePetCategory(editId.value, { name: form.name, sortOrder: form.sortOrder })
      ElMessage.success('已更新')
    } else {
      await addPetCategory({ name: form.name, sortOrder: form.sortOrder })
